    ...
    <v-container height="100%">
        ...
        <v-row class="mt-16">
            ...
            <v-radio-group v-model="selectedCategory" row>
                    <v-btn rounded v-for="item in categoryItems" :key="item.value" :value="item.value" v-radio @click="changeCategory(item.value)" :class="{'darken': selectedCategory === item.value ,'mb-4 w-50 mx-auto grey':true,}">
                    {{ item.text }}
                    </v-btn>
            </v-radio-group>
            </v-col>
