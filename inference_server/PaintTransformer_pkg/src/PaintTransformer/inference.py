...
def partial_render(this_canvas, patch_coord_y, patch_coord_x, stroke_id, patch_size_y, patch_size_x, h, w, param, decision, b, p, meta_brushes):
...
def param2img_serial(
    ...
    if even_idx_y.shape[0] > 0 and even_idx_x.shape[0] > 0:
        for i in range(s):
            canvas = partial_render(
                cur_canvas, even_y_even_x_coord_y, even_y_even_x_coord_x, i, patch_size_y, patch_size_x, h, w, param, decision, b, p, meta_brushes
            )
            if not is_odd_y:
                canvas = torch.cat(
    ...
    if odd_idx_y.shape[0] > 0 and odd_idx_x.shape[0] > 0:
        for i in range(s):
            canvas = partial_render(
                cur_canvas, odd_y_odd_x_coord_y, odd_y_odd_x_coord_x, i, patch_size_y, patch_size_x, h, w, param, decision, b, p, meta_brushes
            )
            canvas = torch.cat(
    ...
    if odd_idx_y.shape[0] > 0 and even_idx_x.shape[0] > 0:
        for i in range(s):
            canvas = partial_render(
                cur_canvas, odd_y_even_x_coord_y, odd_y_even_x_coord_x, i, patch_size_y, patch_size_x, h, w, param, decision, b, p, meta_brushes
            )
            canvas = torch.cat(
                [cur_canvas[:, :, : patch_size_y // 2, : canvas.shape[3]], canvas],
                dim=2,
    ...
    if even_idx_y.shape[0] > 0 and odd_idx_x.shape[0] > 0:
        for i in range(s):
            canvas = partial_render(
                cur_canvas, even_y_odd_x_coord_y, even_y_odd_x_coord_x, i, patch_size_y, patch_size_x, h, w, param, decision, b, p, meta_brushes
            )
            canvas = torch.cat(
                [cur_canvas[:, :, : canvas.shape[2], : patch_size_x // 2], canvas],
                dim=3,
            ...
            if not is_odd_y:
                canvas = torch.cat(
                    [
                        canvas,
                        cur_canvas[:, :, -patch_size_y // 2 :, -canvas.shape[3] :],
                    ],
                    dim=2,
                )
            if is_odd_x:
...
def init(stroke_num: int = 8, model_path: str = "model.pth", mode: str = "large"):
    path = get_path_from_current_file(model_path)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = prepare_infer_model(path, stroke_num, device)
    meta_brushes = make_meta_brushes(device, mode=mode)
...
def inference(
    ...
    with torch.no_grad():
        ...
        frame_list = frame_list[-1].cpu().numpy()
        # frame_list = np.array(frame_list)
        # frame_list = np.ascontiguousarray(frame_list)
        frame_list = frame_list.transpose((1, 2, 0))
        frame_list = (255 * np.clip(frame_list, 0, 1)).astype(np.uint8)
        return Image.fromarray(frame_list)
